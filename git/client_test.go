...
func TestClientClone(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "clone",
			wantCmdArgs: `path/to/git clone github.com/ungtb10d/cli`,
			wantTarget:  "cli",
		},
		{
			name:        "accepts command modifiers",
			mods:        []CommandModifier{WithRepoDir("/path/to/repo")},
			wantCmdArgs: `path/to/git -C /path/to/repo clone github.com/ungtb10d/cli`,
			wantTarget:  "cli",
		},
		{
			name:          "git error",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			target, err := client.Clone(context.Background(), "github.com/ungtb10d/cli", []string{}, tt.mods...)
			assert.Equal(t, tt.wantCmdArgs, strings.Join(cmd.Args[3:], " "))
			if tt.wantErrorMsg == "" {
				assert.NoError(t, err)
			} else {
