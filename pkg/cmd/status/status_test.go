...
func TestNewCmdStatus(t *testing.T) {
	tests := []struct {
		...
		{
			name: "exclude",
			cli:  "-e ungtb10d/cli,cli/go-gh",
			wants: StatusOptions{
				Exclude: []string{"ungtb10d/cli", "cli/go-gh"},
			},
		},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			httpStubs: func(reg *httpmock.Registry) {
				...
				reg.Register(
					httpmock.REST("GET", "repos/ungtb10d/cli/issues/1096"),
		...
		},
		{
			...
			opts: &StatusOptions{
				Exclude: []string{"ungtb10d/cli"},
			},
			// NOTA BENE: you'll see ungtb10d/cli in search results because that happens
		...
		},
		{
			...
			opts: &StatusOptions{
				Exclude: []string{"ungtb10d/cli", "rpd/todo"},
			},
			// NOTA BENE: you'll see ungtb10d/cli in search results because that happens
			// server side and the fixture doesn't account for that
		...
		},
		{
			...
			wantOut: heredoc.Doc(`
				...
				vilmibm/testing#157  yolo             │ ungtb10d/cli#5272  Pin extensions          
				                                      │                                       
				Review Requests                       │ Mentions                              
				ungtb10d/cli#5272  Pin extensions          │ ungtb10d/cli#1096               @jillval...
				                                      │ vilmibm/gh-screensaver#15  a messag...
				                                                                              
				Repository Activity
				rpd/todo#5326         new PR                        Only write UTF-8 BOM on W...
				vilmibm/testing#5325  comment on Ability to sea...  We are working on dedicat...
				ungtb10d/cli#5319          comment on [Codespaces] D...  Wondering if we shouldn't...
				ungtb10d/cli#5300          new issue                     Terminal bell when a runn...
