...
func TestNewCmdSet(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cli:  `-u -bs -r"monalisa/coolRepo,ungtb10d/cli,github/hub" cool_secret`,
			wants: SetOptions{
				SecretName:      "cool_secret",
				Visibility:      shared.Selected,
				RepositoryNames: []string{"monalisa/coolRepo", "ungtb10d/cli", "github/hub"},
				Body:            "s",
			},
		},
		{
		...
		{
			name: "Codespaces org",
			cli:  `random_secret -ocoolOrg -b"random value" -vselected -r"coolRepo,ungtb10d/cli" -aCodespaces`,
			wants: SetOptions{
				SecretName:      "random_secret",
				Visibility:      shared.Selected,
				RepositoryNames: []string{"coolRepo", "ungtb10d/cli"},
				Body:            "random value",
				OrgName:         "coolOrg",
	...
	tests := []struct {
		...
		},
		{
			...
			opts: &SetOptions{
				UserSecrets:     true,
				Visibility:      shared.Selected,
				RepositoryNames: []string{"ungtb10d/cli", "github/hub"},
			},
